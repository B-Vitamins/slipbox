...
The one-particle density for a homogenous gas at temperature \( T \) and with number density \( n \) is (see [[id:c3729898-9aa3-4966-a16c-24216deb2c8d][Equilibrium properties]]) is

\begin{align*}
f_1(\vec{p},\,\vec{q}) = f_1(\vec{p}) = n \bigg(\frac{1}{2 \pi\, m k_B T}\bigg)^{3 / 2} \exp \bigg(-\frac{\big[\vec{p}-\vec{p}_0\big]^2}{2 m k_B T}\bigg).
\end{align*}

Using \( f_1 (\vec{v}) = f_1 (\vec{p}) \lvert D_{\vec{v}} \vec{p} \rvert \) and \( \vec{p} = m \vec{v} \), we have

\[f (\vec{v})= \bigg(\frac{m}{2 \pi \,k_B T}\bigg)^{3 / 2} \exp \bigg(- \frac{m \, v^2}{2 k_B T} \bigg).\]

Note that \( \lvert \mathrm{D}_{\vec{v}} \vec{p} \rvert = m^3 \) and \( \vec{p}_0 = \langle \vec{p} \rangle = 0 \) since the box is immobile.
...
The [[id:ad948e53-1a7b-4c30-81cc-708f6ab74d58][current density]] or flux is \[ J = \frac{N}{A \mathrm{d}t} = \frac{S_{d-1}}{S_d} \cdot \frac{1}{(d-1)} \cdot n \langle v \rangle \]. Since \( S_d = \frac{2 \pi^{d/2}}{(d/2 - 1)!} \), we get \(J = 4^{-1} n \langle v \rangle\). Substituting \(\langle v \rangle = \sqrt{\frac{8 k_B T}{\pi m}}\), we get \(J = n \sqrt{\frac{k_B T}{2 \pi m}}\).
