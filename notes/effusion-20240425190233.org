...
\begin{align*}
f_1(\vec{p},\,\vec{q}, t) & =\bigg\langle\sum_{i=1}^N \delta^3(\vec{p}-\vec{p}_i) \delta^3(\vec{q}-\vec{q}_i)\bigg\rangle \\
& =N \int \prod_{i=2}^N \mathrm{~d}^3 \vec{p}_i \mathrm{~d}^3 \vec{q}_i \rho\big(\vec{p}_1=\vec{p}, \vec{q}_1=\vec{q}, \vec{p}_2, \vec{q}_2, \cdots, \vec{p}_N, \vec{q}_N, t\big),
\end{align*}
