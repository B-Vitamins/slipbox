...
There are three time scales at play (see [[id:2dfdecd5-e088-4eb2-a608-f00b34f3cf76][time scales in the Boltzmann equation]]), the [[id:2dfdecd5-e088-4eb2-a608-f00b34f3cf76][extrinsic time scale]] \( \tau_{U} \), the  [[id:2dfdecd5-e088-4eb2-a608-f00b34f3cf76][collision duration]] \( \tau_{c} \), and the  [[id:2dfdecd5-e088-4eb2-a608-f00b34f3cf76][mean free time]] \( \tau_{\times} \). The typical values for a dilute gas are \( \tau_U \approx 10^{-5} s\), \( \tau_c \approx 10^{-12} s\), and \( \tau_\times \approx 10^{-8} s\).

For short-range interactions, \(\tau_{\times} \) is much longer than \(\tau_c\), and the collision terms on the right-hand side of Eqs. (3.28) are smaller by a factor of \(n d^3 \approx \left(10^{26} \, \text{m}^{-3}\right)\left(10^{-10} \, \text{m}\right)^3 \approx 10^{-4}\).
