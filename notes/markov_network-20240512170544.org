...
In the undirected graph above every path from any node in set \(A\) to any node in set \(B\) passes through at least one node in set \(C\). Consequently the conditional independence property \(A \perp\!\!\!\perp B \mid C\) holds for any probability distribution described by this graph. Note that this is exactly the same as the d-separation criterion except that there is no 'explaining away' phenomenon. /Testing for conditional independence in undirected graphs is therefore simpler than in directed graphs./
...
The [[id:679ef603-2b30-4dab-9c68-bd4921963738][Markov blanket]] for an undirected graph takes a particularly simple form, because a node will be conditionally independent of all other nodes conditioned only on the neighboring nodes, as illustrated below
